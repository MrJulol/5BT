...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
   if (isset($_POST['note'])) {
      try {
         ...
         // Set a cookie to remember the last note edited by the user
         setcookie('last_note', $_POST['note'], time() + (86400 * 30), "/"); // 86400 = 1 day
...
<body>
   <div class="note-container">
      <h1>Notizbuch für <?php echo htmlspecialchars($username); ?></h1>
      <p>Letzte Änderung: <?php echo htmlspecialchars($lastModified); ?></p>
      <form method="post">
         <textarea name="note" rows="10" cols="30"><?php echo htmlspecialchars($note); ?></textarea>
         <br>
         <input type="submit" value="Speichern">
      </form>
      <?php
      if (isset($_COOKIE['last_note'])) {
         echo "<p>Letzte Notiz: " . htmlspecialchars($_COOKIE['last_note']) . "</p>";
      }
      ?>
   </div>
</body>
